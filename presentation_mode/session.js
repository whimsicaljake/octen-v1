...
anchorElement.href = '/presentation_mode/full.html'+currentHash;
...
saveVersionButton.addEventListener("click", () => {
    ...
    versionsRef.once("value", (snapshot) => {
        ...
        saveCurrentVersionFunction(versionName);
    ...
    if (user) {
        ...
        const photoURL = user.photoURL;
         ...
         if (message.startsWith('@gpt')) {
            ...
            // Check if the message starts with @analyze
...
messageInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter" && !event.shiftKey) {
        ...
        if (messageInput.value.trim() !== "") {
            // Check if the message starts with @analyze
            if (messageInput.value.trim().startsWith('@analyze')) {
                analyzeCode();
            } else {
...
function analyzeCode() {
    const codeToAnalyze = messageInput.value.substring("@analyze".length).trim();
    // Add your code analysis logic here
    const analysisResults = performCodeAnalysis(codeToAnalyze);

    // Send the analysis results as a chat message
    const user = auth.currentUser;
    if (user) {
        const displayName = user.displayName || "Anonymous";
        const photoURL = user.photoURL;
        ...
        chatRef.push({
            ...
            photoURL: 'https://cdn.dribbble.com/users/5302479/screenshots/14627686/media/7cbe6a206c018291b6030f3eee9723de.png?resize=768x576&vertical=center'
        });
...
function performCodeAnalysis(code) {
    // Regular expression to identify missing semicolons
    const missingSemicolonRegex = /;(?![^{}]*[}])/g;

    // Regular expression to identify undefined variables
    const undefinedVariableRegex = /\b(?:var|let|const)\s+(\w+)\s*=\s*undefined\b/g;
tyy
    // Perform analysis
    let analysisResults = "";

    const getSnippet = (index, length) => {
        const start = Math.max(0, index - 20);
        const end = Math.min(code.length, index + length + 5);
        return code.substring(start, end);
    };

    const missingSemicolonMatches = code.matchAll(missingSemicolonRegex);
    for (const match of missingSemicolonMatches) {
        const snippet = getSnippet(match.index, 0);
        analysisResults += `Missing semicolon. Snippet: ${snippet}\n`;
    }

    const undefinedVariableMatches = code.matchAll(undefinedVariableRegex);
    for (const match of undefinedVariableMatches) {
        const snippet = getSnippet(match.index, match[0].length);
        analysisResults += `Undefined variable. Snippet: ${snippet}\n`;
    }

    if (analysisResults === "") {
        analysisResults = "No issues found in the provided code.";
    }
